...
class Category
{
    ...
    function selectAll()
    {
        $query = "select * from gatagorey  "; // Query to select all data about category table
    ...
    function add($data)
    {
        if (empty($data['cate-name']) || empty($data['cate-description'])) // For make sure that post value is not empty
        {
            ...
            $query = " insert into gatagorey (name,des) values('$cateName','$cateDes')"; //Query for insert values to product table
            $result = mysqli_query($this->con, $query);
    ...
    function update($data, $id)
    {
        $cateName = $data['cate-name'];
        $cateDes = $data['cate-description'];
        $query = " update gatagorey set name = '" . $cateName . "', des='" . $cateDes . "' where id='" . $id . "'"; // this query for update table category
        $result = mysqli_query($this->con, $query);
        if ($result) //check query 
        {
    ...
    function delete($id)
    {
        $query = " delete from gatagorey where id = '" . $id . "'"; // this query for delete record from table category
        $result = mysqli_query($this->con, $query);
        if ($result) {
