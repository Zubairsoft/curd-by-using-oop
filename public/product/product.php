...
class Product
{
    ...
    function add($data)
    {
        if (empty($data['pro-name']) || empty($data['pro-des']) || empty($data['price']) || empty($data['category'])) // For make sure that post value is not empty
        {
            ...
            if (in_array($ex, $file_ex)) {
                ...
                {
                    header("location:index.php"); // If true redirct to index.php
                } else {
    ...
    function update($data, $id,$img)
    {
        ...
        $query = "update product set prodName ='" . $proName . "', des='" . $proDes . "', price ='" . $price . "', image ='" . $img . "', gataID ='" . $gata . "' where id='" . $proID . "'"; //this query for update table products
        $result = mysqli_query($this->con, $query);
